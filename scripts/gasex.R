...
gasex <- read.csv("raw_data/gasexchange_master.csv")
        ...
        gasex$uniqueID <- paste(gasex$species, gasex$site, sep = "-")
...
axis(1, labels=FALSE)
text(x=1:6, y= par("usr")[3] - 2, labels = uniqueID_label, xpd=NA, srt=30, adj=.565, cex=1.25)
...
boxplot(gsw ~ uniqueID, data=gasex,varwidth=TRUE,xaxt='n',
        ...
        boxlwd=2, whisklwd=2,staplelwd=2)
axis(1, labels=FALSE)
text(x=1:6, y= par("usr")[3]-.035 , labels = uniqueID_label, xpd=NA, srt=30, adj=.565, cex=1.25)
...
boxplot(ITE ~ uniqueID, data=gasex,varwidth=TRUE,xaxt='n',
        ...
        boxlwd=2, whisklwd=2,staplelwd=2)
axis(1, labels=FALSE)
text(x=1:6, y= par("usr")[3]-.5 , labels = uniqueID_label, xpd=NA, srt=30, adj=.565, cex=1.25)
...
gasex_agg <- doBy::summaryBy(A + gsw + ITE ~ species + site + Date, data=gasex, 
                             FUN=c(mean, se))

maple <- gasex_agg[gasex_agg$species == "m",]
dogwood <- gasex_agg[gasex_agg$species == "d",]
hawthorn <- gasex_agg[gasex_agg$species == "h",] 
...
xlim2 <- as.Date(strptime("07-5-2022", format = "%m-%d-%Y", tz=""))
